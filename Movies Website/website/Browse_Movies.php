...
<div class="title">
<span id="font2">Movie Information</span>
</div>
...
print '<strong>Movie</strong><br><br>';
...
$query = 'select CONCAT(d.first, " ", d.last) from Director d, MovieDirector md where d.id=md.did AND md.mid='.$id.';';
...
$query = 'select CONCAT(a.first, " ", a.last), ma.role, a.id from Actor a, MovieActor ma where a.id=ma.aid AND ma.mid='.$id.' order by a.first;';
...
print '<hr style="margin-top: 3%; color: lightgray;">';

print '<strong>Actors in this Movie</strong><br><br>';
...
$query = 'select AVG(rating) from Review where mid='.$id.' group by mid;';
...
print '<strong>Movie Reviews</strong><br>';

print '<a href="Add_Comments.php" class="button2">Add Your Review Now!</a><br><br>';

if ($row[0] != NULL)
{
    print 'Average Review Rating: '.$row[0].'<br><br>';
    ...
    while ($row = mysql_fetch_row($result))
    {
        print $row[0].' rates this movie '.$row[2].'/5<br>';
        print 'Comments: '.$row[3].'<br>';
        print 'Reviewed on '.$row[1].'<br><br>';   
    } 
}
else
{
    print 'No reviews have been added';
}
